...
from src.quizzes.admin import TaskItemInline
...
class QuizAdmin(SortableAdminMixin, admin.ModelAdmin):
    ...
    fields = ('show', 'title', 'slug', 'lang', 'author', 'end_time', 'about', 'content')
    ...
    def get_inline_instances(self, request, obj=None):
        ...
        if not obj:
            for inline in inline_instances:
                if inline.__class__ == TaskItemInline:
    ...
    def get_changeform_initial_data(self, request):
        ...
        initial.update({
            'author': request.user,
            'order_key': Quiz.objects.all().count() + 1
        })
        return initial
