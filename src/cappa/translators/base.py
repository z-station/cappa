...
class DockerProvider(BaseProvider):
    ...
    @classmethod
    def get_docker_container(cls) -> Container:

        """ Возвращает контейнер транслятора """

        if settings.ENV_SUFFIX:
            container_name = f'{cls.get_translator_name()}-{settings.ENV_SUFFIX}'
        else:
            container_name = cls.get_translator_name()
        return cls.client.containers.get(container_name)
