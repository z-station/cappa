# -*- coding: utf-8 -*-
# Generated by Django 1.11.29 on 2025-05-03 14:38
from __future__ import unicode_literals

import app.common.fields
from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import tinymce.models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='News',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('show', models.BooleanField(default=False, verbose_name='показывать')),
                ('title', models.CharField(max_length=255, verbose_name='заголовок')),
                ('date', models.DateField(verbose_name='дата создания')),
                ('content', tinymce.models.HTMLField(blank=True, null=True, verbose_name='описание')),
                ('card_type', models.CharField(choices=[('horizontal', 'горизонтальная'), ('vertical', 'вертикальная')], default='horizontal', help_text='у горизонтальной карточки фото расположено слева от текста, у вертикальной карточки фото находится над текстом', max_length=10, verbose_name='тип карточки')),
                ('image', app.common.fields.AnyExtFileBrowseField(blank=True, max_length=1000, null=True, verbose_name='изображение')),
                ('author', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to=settings.AUTH_USER_MODEL, verbose_name='автор')),
            ],
            options={
                'verbose_name': 'запись на стене',
                'verbose_name_plural': 'записи на стене',
                'ordering': ['-date'],
            },
        ),
    ]
