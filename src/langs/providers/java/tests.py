...
class ProviderTestCase(TestCase):
    ...
    def test_provider(self):
        ...
        """ Прогон java по тестовой выборке задач """

        print('===> RUN Java tests')
        provider = Lang.objects.get(provider_name=langs.JAVA).provider

        # Задачи, тесты которых должны закончиться успехом
        for task in Task.objects.filter(lang=langs.JAVA, tags__name='success'):
            solution = task.solution_examples.filter(lang=langs.JAVA).first()
        ...
        for task in Task.objects.filter(lang=langs.JAVA, tags__name='unluck'):
            solution = task.solution_examples.filter(lang=langs.JAVA).first()
            tests_result = provider.check_tests(content=solution.content, task=task)
            self.assertFalse(
                expr=tests_result['success'],
