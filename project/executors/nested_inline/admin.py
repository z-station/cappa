...
class NestedModelAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {
            "all": ('admin/executors/css/forms-nested.css',)
        }
        js = ('admin/executors/js/inlines-nested.js',)

    def save_formset(self, request, form, formset, change):
        """
            Вызывается при сохранении источника для сохранения и1у
        """
    ...
    def save_related(self, request, form, formsets, change):
        """
            После сохранения источника сохранить и1у
        """
        form.save_m2m()
    ...
    def add_nested_inline_formsets(self, request, inline, formset, depth=0):
        ...
        """ Для каждой и1у найти и2у и записать в nested_formsets формы"""
    ...
    def wrap_nested_inline_formsets(self, request, inline, formset):
        ...
        """ Для и2у создать обертку вокруг встроенного набора форм для использования в системе администратора """
    ...
    def formset_has_nested_data(self, formsets):
        """ рекурсивно проверяет наличие данных в формсете """
        for formset in formsets:
...
class NestedInline(InlineModelAdmin):
    ...
    def media(self):
        ...
        if self.prepopulated_fields:
            js.extend(['admin/js/urlify.js', 'admin/js/prepopulate.js', ])
...
class NestedStackedInline(NestedInline):
    template = 'admin/executors/nested_inline/stacked-nested.html'


class NestedTabularInline(NestedInline):
    template = 'admin/executors/nested_inline/tabular-nested.html'
